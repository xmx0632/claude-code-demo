...
if [ -f "h2/data/todolist.mv.db" ]; then
    SIZE=$(du -sh h2/data/todolist.mv.db | cut -f1)
    echo "数据库大小: $SIZE"
    echo "创建时间: $(stat -f '%Sm' -t '%Y-%m-%d %H:%M:%S' h2/data/todolist.mv.db)"
    echo "修改时间: $(stat -f '%Mm' -t '%Y-%m-%d %H:%M:%S' h2/data/todolist.mv.db)"
fi
